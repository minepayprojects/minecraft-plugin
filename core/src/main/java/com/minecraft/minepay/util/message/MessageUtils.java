package com.minecraft.minepay.util.message;
...
public enum MessageUtils {
    ...
    MessageUtils(final String s, final int n, final char character, final int length) {
    ...
    public static MessageUtils getDefaultFontInfo(final char c) {
        MessageUtils[] values;

        for (int length = (values = values()).length, i = 0; i < length; ++i) {
            final MessageUtils dFI = values[i];
    ...
    public static String makeCenteredMotd(final String message) {
        ...
        for (int i = (arrayOfChar = message.toCharArray()).length, j = 0; j < i; ++j) {
            ...
            if (c == '§') {
                ...
                final MessageUtils dFI = getDefaultFontInfo(c);
        ...
        final int spaceLength = MessageUtils.SPACE.getLength() + 1;
    ...
    public static String makeFormattedProgress(int current, int total, String color, String symbol) {
        float percent = (float) current / total;

        int progress = (int) (10 * percent);

        int count = 10 - progress;
        ...
        return color + Strings.repeat(symbol, Math.min(progress, 10))
                + "§7" + Strings.repeat(symbol, count)
                + " " + (percentage + "%");
    }
}
